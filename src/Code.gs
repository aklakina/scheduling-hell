...
function dailySchedulingCheck() {
  if (!CONFIG.triggers.daily.enabled) {
    Logger.log('Daily scheduling check is disabled in configuration.');
    return;
  }
...
function checkAndScheduleEventsWithTrigger(triggerType = 'monthly') {
  ...
  // --- NEW: Use trigger-aware processing window calculation ---
  const today = new Date();
  today.setHours(0, 0, 0, 0);

  const windowInfo = calculateProcessingWindow(triggerType, today);
  const processingStartDate = windowInfo.processingStartDate;
  const processingEndDate = windowInfo.processingEndDate;
  const windowDays = windowInfo.windowDays;

  Logger.log(`Processing window (${triggerType}): ${processingStartDate.toLocaleDateString()} to ${processingEndDate.toLocaleDateString()} (${windowDays} days)`);
  ...
  // --- Create data structure to collect notifications by date ---
  const notificationsByDate = {};
  const remindersByDate = {};
  ...
  for (const week in eventsByWeek) {
    ...
    // Check for events that could meet duration threshold with optimal player combination
    eventsByWeek[week].forEach((event, eventIndex) => {
      const status = event.rowData[statusColumnIndex - 1];
      ...
      if (status === CONFIG.messages.status.awaitingResponses || status === CONFIG.messages.status.readyForScheduling) {
        ...
        // NEW: Check if this trigger type allows duration warnings
        if (CONFIG.triggers[triggerType].allowDurationWarnings &&
            optimalCombination.players.length >= Math.ceil(numPlayers * CONFIG.playerCombinationThresholdPercentage) &&
            optimalCombination.duration >= CONFIG.minEventDurationHours &&
            optimalCombination.restrictingPlayers.length > 0) {
    ...
    // If a best event was found, schedule it and update status for the whole week
    if (bestEvent) {
      // NEW: Check if this trigger type allows event notifications
      if (CONFIG.triggers[triggerType].allowEventNotifications) {
        const dateString = bestEvent.date.toLocaleDateString();
        if (!notificationsByDate[dateString]) {
          notificationsByDate[dateString] = {
            date: bestEvent.date,
      ...
      // NEW: Check if this trigger type allows reminders
      if (CONFIG.triggers[triggerType].allowReminders) {
        eventsByWeek[week].forEach(event => {
          const status = event.rowData[statusColumnIndex - 1];
          if (status === CONFIG.messages.status.awaitingResponses) {
            const eventDate = new Date(event.rowData[CONFIG.dateColumn - 1]);
  ...
  // Archive old data and create new date rows (only for monthly triggers to avoid conflicts)
